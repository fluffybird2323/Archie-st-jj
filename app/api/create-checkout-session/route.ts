...
export async function POST(request: NextRequest) {
  try {
    ...
    const { priceId, productName, productImage, locale = "en" } = await request.json()
    ...
    const priceInSmallestUnit = Math.round(localPrice * (currency.code === "JPY" ? 1 : 100)) // JPY doesn't use cents

    // Configure payment methods based on locale
    const paymentMethodTypes: string[] = ["card"]

    // Add Konbini for Japanese customers
    if (locale === "ja") {
      paymentMethodTypes.push("konbini")
    }

    // Add other regional payment methods
    if (locale === "zh") {
...
// Add a GET method to check if Stripe is configured (optional)
